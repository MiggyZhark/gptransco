...
class DispatcherProfileScreen extends StatefulWidget {
  final Map<String, dynamic> dispatcherProfileData;
  const DispatcherProfileScreen({super.key, required this.dispatcherProfileData});

  @override
  State<DispatcherProfileScreen> createState() => _ProfileScreenState();
}

class _ProfileScreenState extends State<DispatcherProfileScreen> {
  ...
  void initState() {
    super.initState();
    nameController.text = widget.dispatcherProfileData['driverName'] ?? 'N/A';
    mobileController.text = widget.dispatcherProfileData['mobileNumber'] ?? 'N/A';
    addressController.text = widget.dispatcherProfileData['address'] ?? 'N/A';
  }
  ...
  Future<void> _changeProfileImage() async {
    ...
    if (downloadUrl != null) {
      ...
      if (currentUser != null) {
        await _firestore.collection('Driver').doc(currentUser.uid).update({
          'driverImage': downloadUrl,
        });

        setState(() {
          widget.dispatcherProfileData['driverImage'] =
  ...
  Future<void> _saveProfileChanges() async {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        setState(() {
          widget.dispatcherProfileData['driverName'] = nameController.text;
          widget.dispatcherProfileData['mobileNumber'] = mobileController.text;
          widget.dispatcherProfileData['address'] = addressController.text;
        });
      }
    } catch (e) {
            ...
            Stack(
              children: [
                CircleAvatar(
                  backgroundColor: Colors.white,
                  radius: 55,
                  backgroundImage: widget.dispatcherProfileData['driverImage']
                      .startsWith('http')
                      ? NetworkImage(widget.dispatcherProfileData['driverImage'])
                      : AssetImage(widget.dispatcherProfileData['driverImage'])
                  as ImageProvider,
                ),
                Positioned(
                  bottom: 0,
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      children: <Widget>[
                        ...
                        ProfileTextBox(
                          contentText: widget.dispatcherProfileData['driverName'] ?? 'N/A',
                        ),
                        const ProfileTextLabel(
                          labelText: 'Email',
                        ),
                        ProfileTextBox(
                          contentText: widget.dispatcherProfileData['email'] ?? 'N/A',
                        ),
                        const ProfileTextLabel(
                          labelText: 'Mobile Number',
                        ),
                        ProfileTextBox(
                          contentText:
                          widget.dispatcherProfileData['mobileNumber'] ?? 'N/A',
                        ),
                        const ProfileTextLabel(
                          labelText: 'Address',
                        ),
                        ProfileTextBox(
                          contentText:
                          widget.dispatcherProfileData['address'] ?? 'N/A',
                        ),
                        Padding(
