...
class DriverProfileScreen extends StatefulWidget {
  ...
  const DriverProfileScreen({super.key, required this.userProfileData});

  @override
  State<DriverProfileScreen> createState() => _ProfileScreenState();
}
...
class _ProfileScreenState extends State<DriverProfileScreen> {
  ...
  Future<void> _changeProfileImage() async {
    ...
    String? downloadUrl = await _imageService.driverUploadImageToStorage(imageFile);
    if (downloadUrl != null) {
      // Step 3: Save the new image URL to Firestore
      await _imageService.saveDriverImageUrlToFirestore(downloadUrl);

      setState(() {
        widget.userProfileData['driverImage'] = downloadUrl; // Update local state
      });
    }
        ...
        body: Column(
          ...
          children: [
            CircleAvatar(backgroundColor: Colors.white,
              radius: 55,
              backgroundImage: widget.userProfileData['driverImage'].startsWith('http')
                  ? NetworkImage(widget.userProfileData['driverImage'])
                  : AssetImage(widget.userProfileData['driverImage']) as ImageProvider,
            ),
            const SizedBox(
              height: 5,
            ),
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        children: <Widget>[
                          ...
                          ProfileTextBox(
                            contentText: widget.userProfileData['driverName'] ?? 'N/A',
                          ),
                          const ProfileTextLabel(
                            labelText: 'Email',
                          ),
